export type ReportType = 'due' | 'collected' | 'overdue' | 'future' | 'expenses'
export type ReportRow = Record<string, string | number | null>
export const REPORT_LABELS: Record<ReportType, string> = { due: 'Due Bills', collected: 'Collected', overdue: 'Overdue', future: 'Bills Expected in Future', expenses: 'Expenses' }
...
export function reportCsv(type: ReportType, rows: ReportRow[]) {
  ...
  return '\uFEFF' + [columns.map(c => escape(c.label)).join(','), ...rows.map(r => columns.map(c => escape(reportCell(r,c.key))).join(','))].join('\r\n')
}
