...
export function periodRange(
  ...
  const month = Number(local.slice(5, 7)) - 1
...
export function reportColumns(type: ReportType) {
  if (type === 'income-statement') {
    return [
      { key: 'label', label: 'Income / Expense' },
      { key: 'detail', label: 'Amount (NGN)' },
  ...
  if (type === 'expenses') {
    return [
      { key: 'house', label: 'Category' },
      { key: 'charge', label: 'Description' },
      { key: 'date', label: 'Date' },
  ...
  return [
    { key: 'house', label: 'Target' },
    { key: 'charge', label: 'Charge' },
    { key: 'period', label: 'Period' },
    ...(type === 'collected'
      ? [
          { key: 'date', label: 'Paid date (WAT)' },
...
export function reportCell(row: ReportRow, key: string) {
  ...
  if (key === 'date' && String(value).includes('T')) {
...
export function reportCsv(
  ...
  return (
    ...
    [
      ...
      ...rows.map((row) =>
        columns
          .map((column) => escape(reportCell(row, column.key)))
          .join(',')
      ),
    ].join('\r\n')
  )
